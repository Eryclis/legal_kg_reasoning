// =============================================================
// Legal KG — CDC Arts. 1–30 | Neo4j Aura Import Script
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/nodes_LegalProvision.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/nodes_LegalSubProvision.csv' AS row
...
// ── 4. Nodes: Case ─────────────────────────────────────
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/nodes_Case.csv' AS row
CREATE (:Case {
  nodeId: row.nodeId,
  title: row.title,
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/nodes_RepetitiveTopic.csv' AS row
...
// ── 6. Nodes: STJThesis ─────────────────────────────────────
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/nodes_STJThesis.csv' AS row
CREATE (:STJThesis {
  nodeId: row.nodeId,
  edition: row.edition,
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/nodes_ConstitutionalControl.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/rel_INTERPRETED_BY_BINDING_THESIS.csv' AS row
MATCH (a {nodeId: row.start_id})
MATCH (b {nodeId: row.end_id})
CREATE (a)-[:INTERPRETED_BY_BINDING_THESIS {weight: toFloat(row.weight)}]->(b);

// ── 11. Rel: INTERPRETED_BY_LEADING_CASE ─────────────────
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/rel_INTERPRETED_BY_LEADING_CASE.csv' AS row
MATCH (a {nodeId: row.start_id})
MATCH (b {nodeId: row.end_id})
CREATE (a)-[:INTERPRETED_BY_LEADING_CASE {weight: toFloat(row.weight)}]->(b);

// ── 12. Rel: INTERPRETED_BY_ISOLATED_CASE ─────────────────
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/rel_INTERPRETED_BY_ISOLATED_CASE.csv' AS row
MATCH (a {nodeId: row.start_id})
MATCH (b {nodeId: row.end_id})
CREATE (a)-[:INTERPRETED_BY_ISOLATED_CASE {weight: toFloat(row.weight)}]->(b);

// ── 13. Rel: SUMMARIZED_BY_STJ_THESIS ─────────────────
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/rel_SUMMARIZED_BY_STJ_THESIS.csv' AS row
MATCH (a {nodeId: row.start_id})
MATCH (b {nodeId: row.end_id})
CREATE (a)-[:SUMMARIZED_BY_STJ_THESIS {weight: toFloat(row.weight)}]->(b);

// ── 14. Rel: HAS_SUB_PROVISION ─────────────────
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/rel_HAS_SUB_PROVISION.csv' AS row
MATCH (a {nodeId: row.start_id})
MATCH (b {nodeId: row.end_id})
CREATE (a)-[:HAS_SUB_PROVISION {weight: toFloat(row.weight)}]->(b);

// ── 15. Rel: HAS_INCISE ─────────────────
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/rel_HAS_INCISE.csv' AS row
MATCH (a {nodeId: row.start_id})
MATCH (b {nodeId: row.end_id})
CREATE (a)-[:HAS_INCISE {weight: toFloat(row.weight)}]->(b);

// ── 16. Rel: CITES_ARTICLE ─────────────────
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/rel_CITES_ARTICLE.csv' AS row
MATCH (a {nodeId: row.start_id})
MATCH (b {nodeId: row.end_id})
CREATE (a)-[:CITES_ARTICLE {weight: toFloat(row.weight)}]->(b);

// ── 17. Rel: CITES_EXTERNAL ─────────────────
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/Eryclis/legal_kg_reasoning/main/01_kg_pilot/output/cdc_art1_30/neo4j_data_importer/rel_CITES_EXTERNAL.csv' AS row
MATCH (a {nodeId: row.start_id})
MATCH (b {nodeId: row.end_id})
